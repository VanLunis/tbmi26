...
T = xTrain;
Weight = ones(size(xTrain,2))/size(xTrain,2);
Classification = zeros(size(xTrain));
Error = zeros(size(xTrain));

for tValueRow = 1:size(T,1)
     for xRow = 1:size(xTrain,1)
         Classification(xRow,:) = WeakClassifier(T(tValue), 1, xTrain(xRow));
         Error(xRow,:) = WeakClassifierError(Classification(xRow,:), Weight(xRow,:), yTrain);
     end
end
