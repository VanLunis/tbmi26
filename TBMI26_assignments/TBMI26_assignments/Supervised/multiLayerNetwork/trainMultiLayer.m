...
[Ytraining, ~, Utraining, Straining] = runMultiLayer(Xtraining, W0, V0);
...
for n = 1:numIterations
    [Ytraining, ~, Utraining, Straining] = runMultiLayer(Xtraining, Wout, Vout);
    
    A = Ytraining - Dtraining;
    grad_v = (2/length(Xtraining))*A.*(ones(size(Ytraining))-(tanh(Ytraining).^2))*(Utraining'); %Calculate the gradient for the output layer
    %grad_v = (2/length(Xtraining))*(Ytraining-Dtraining)*(Utraining'); %Calculate the gradient for the output layer
    A = Vout'*(Ytraining-Dtraining);
    %grad_w = (2/length(Xtraining))*A(2:end,:).*(ones(size(Straining(2:end,:)))-(tanh(Straining(2:end,:)).^2))*(Xtraining'); %..and for the hidden layer.
    grad_w = (2/length(Xtraining))*A(2:end,:).*(ones(size(Straining(2:end,:)))-(tanh(Straining(2:end,:)).^2))*((Xtraining')); %..and for the hidden layer.
%.*(ones(size(Ytraining))-(tanh(Ytraining).^2))'
